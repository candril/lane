# lane packaged from its GitHub release binary — the same .gz and SHA256SUMS the curl
# installer and the Homebrew formula verify against. release.json is written by the release
# workflow the moment a release is published, so `nix run github:candril/lane` is the
...
{
  description = "lane — packaged from its GitHub releases";
  ...
  outputs = { self, nixpkgs }:
    ...
    in
    {
      packages = forAll (pkgs: system:
        ...
        rec {
          default = lane;
          lane = pkgs.stdenvNoCC.mkDerivation {
            pname = "lane";
            version = rel.version;

            src = pkgs.fetchurl {
              url = "https://github.com/candril/lane/releases/download/v${rel.version}/lane-${t}.gz";
              sha256 = rel.sha256.${t};
            };
            dontUnpack = true;
            ...
            installPhase = ''
              runHook preInstall
              mkdir -p $out/bin
              gunzip -c $src > $out/bin/lane
              chmod +x $out/bin/lane
              ${lib.optionalString (runtime != [ ]) ''
                wrapProgram $out/bin/lane --prefix PATH : ${lib.makeBinPath runtime}
              ''}
              runHook postInstall
            '';

            meta = {
              description = "Terminal Kanban board for Jira";
